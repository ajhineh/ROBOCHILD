...
### نحوه اجرا:
جهت اجرای ارزیابی جدید با داده‌های واقعی دوره مشخص شده (مثلاً برای ارزهای SOL یا POPCAT):
```bash
# اجرای ارزیاب روی ارز POPCAT
python -m src.analysis.training_evaluator popcat

# اجرای ارزیاب روی ارز SOL
python -m src.analysis.training_evaluator sol
```
> [!IMPORTANT]
> **شرط استفاده از داده‌های واقعی**: این ارزیاب داده‌های خام را به صورت مستقیم از API صرافی بایننس دانلود می‌کند و فاقد فالبک داده‌های شبیه‌سازی شده (Synthetic Data) است. در صورت عدم دسترسی به شبکه یا مسدود بودن دسترسی صرافی، فرآیند ارزیابی متوقف شده و خطای صریح `RuntimeError` صادر خواهد شد.

همچنین می‌توانید از دکمه **«شروع فرآیند آنالیزور مدل»** در داشبورد مدیریت وب پروژه استفاده نمایید.
